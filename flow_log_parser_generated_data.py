...
# Loading lookup table generated
with open('lookup_table_generated.csv', 'r', newline='', encoding='utf-8-sig') as file:
...
with open('flow_logs_generated.txt', mode='r') as file:
    for line in file:
        process_line_and_update_counters(line)

# Writing the new generated data output files
with open('output_tag_count_generated.csv', 'w', newline='') as file:
    writer = csv.DictWriter(file, fieldnames=["Tag", "Count"])
    writer.writeheader()
    for tag, count in tag_counter.items():
        writer.writerow({"Tag": tag, "Count": count})

with open('output_port_protocol_count_generated.csv', 'w', newline='') as file:
